    ...
    Object.keys(nameMap).forEach(function (name) {
      ...
      matches.forEach(function (service) {
        ...
        if (self._serviceMap[service.fqdn])
        {
          ...
          {
            self._removeService (service.fqdn, false);
          }
          else
          {
            return;
...
Browser.prototype._removeService = function (fqdn, emit=true) {
  var service, index
  ...
  if (emit!==false) this.emit('down', service)
}
...
function buildServicesFor (name, packet, txt, referer) {
  ...
  return records
    ...
    .map(function (ptr) {
      ...
      records
        .filter(function (rr) {
          // console.log (rr.name+' '+service.host+' '+rr.type);
